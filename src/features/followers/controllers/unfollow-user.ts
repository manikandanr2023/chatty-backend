...
export class Remove {
  public async follower(req: Request, res: Response): Promise<void> {
    ...
    const removeFollowerFromCache: Promise<void> = followerCache.removeFollowersFromCache(
      `following:${req.currentUser!.userId}`,
      followeeId
    );

    const removeFolloweeFromCache: Promise<void> = followerCache.removeFollowersFromCache(`followers:${followeeId}`, followerId);
