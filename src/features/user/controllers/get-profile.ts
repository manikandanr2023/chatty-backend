...
export class Get {
  ...
  public async profileByUserId(req: Request, res: Response): Promise<void> {
    ...
    const cachedUsers: IUserDocument = (await userCache.getUserFromCache(userId)) as IUserDocument;
    const existingUser: IUserDocument = cachedUsers ? cachedUsers : await userService.getUserById(userId);
  ...
  public async profile(req: Request, res: Response): Promise<void> {
    const cachedUsers: IUserDocument = (await userCache.getUserFromCache(`${req.currentUser!.userId}`)) as IUserDocument;
    const existingUser: IUserDocument = cachedUsers ? cachedUsers : await userService.getUserById(`${req.currentUser!.userId}`);
    res.status(HTTP_STATUS.OK).json({
      message: "Get user profile by id",
      user: existingUser
    });
