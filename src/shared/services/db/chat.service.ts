...
class ChatService {
    ...
    const messages: IMessageData[] = await MessageModel.aggregate([
      ...
      },
      {
        $project: {
          ...
          receiverId: "$resulst.receiverId",
          receiverUsername: "$result.receiverUsername",
          receiverAvatarColor: "$result.receiverAvatarColor",
          receiverProfilePicture: "$result.senderUsername",
  ...
  public async markMesssagesAsRead(senderId: ObjectId, receiverId: ObjectId): Promise<void> {
    const query = {
      $or: [
